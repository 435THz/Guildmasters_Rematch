...
function guildmaster_summit.Enter(map)
  ...
  if SV.guildmaster_summit.GameComplete and not SV.guildmaster_summit.rematch then
    ...
    elseif SV.guildmaster_summit.BossPhase == 3 then
      if SV.guildmaster_summit.rematch then
        guildmaster_summit.PostRematch()
      else
...
function guildmaster_summit.PostBattle()
  ...
  if GAME:InRogueMode() then
      ...
      guildmaster_summit.can_skip = SV.guildmaster_summit.GameComplete
	  GAME:WaitFrames(180)
...
function guildmaster_summit.Summit_Action(obj, activator)
  ...
  if COMMON.GetPlayerItemCount("orb_revival", true)>0 then
    local continue = guildmaster_summit.Rematch_flow()
...
function guildmaster_summit.Rematch_flow()
  choices = {"Watch the sunset",
             "Rematch",
             "Leave"}

  UI:ResetSpeaker()
  UI:BeginChoiceMenu("What will you do?", choices, 1, 3)
  UI:WaitForChoice()
  result = UI:ChoiceResult()
  if result == 1 then
    return true
  elseif result == 2 then
    local orb = RogueEssence.Dungeon.InvItem("orb_revival")
    UI:WaitShowDialogue("You will need to offer a "..orb:GetDisplayName().." from your Bag or Storage.[pause=20] It will be lost even if you lose the fight.")
    UI:ChoiceMenuYesNo("Is that ok with you?", true)
    UI:WaitForChoice()
    ch = UI:ChoiceResult()
    if ch then
      ...
      GAME:EnterDungeon('guildmaster_island', 0, 8, 0, RogueEssence.Data.GameProgress.DungeonStakes.Progress, true, false)
    end
  end
  return false
end
  
function guildmaster_summit.PostRematch()
  SV.guildmaster_summit.rematch = false
  ...
  UI:SetSpeaker(xatu)
  UI:WaitShowDialogue("You have done well to defeat us.[pause=20] But I also see you already bear the mark of Guildmaster.")
  GAME:WaitFrames(10)
  UI:SetSpeaker(lucario)
  UI:WaitShowDialogue("We have no more glory or status to give you...")
  GAME:WaitFrames(10)
  UI:SetSpeaker(wigglytuff)
  ...
  GAME:EndDungeonRun(RogueEssence.Data.GameProgress.ResultType.Cleared, 'guildmaster_island', -1, 8, 0, false, false)
  COMMON.RespawnAllies()
  guildmaster_summit.SetupNpcs()
  GROUND:Hide("Xatu")
  GROUND:Hide("Lucario")
